...
const messages = {
  ...
  portal: {
    ...
    publicZone: { title: '공용 지식 구역', empty: '게시된 공용 지식 공간이 없습니다.', types: { standards: '표준', templates: '템플릿', training: '교육', practices: '모범 사례' } }, navigationTitle: 'IPD 지식 탐색',
    ...
    categories: { insight_domain: '인사이트', market_customer: '시장 및 고객', requirements_management: '요구사항 관리', architecture_design: '아키텍처 및 설계', development_assets: '개발 자산', test_quality: '테스트 및 품질', operations_lifecycle: '운영 및 수명주기', process_governance: '프로세스 거버넌스', public_knowledge: '공용 지식' },
    stages: {
  ...
  usageAnalytics: {
	...
	tokenScopePhase2: 'Chat, Agent, 주요 백그라운드 Chat 처리, Memory 및 Wiki provider usage가 포함됩니다. Embedding, Rerank, VLM, ASR은 아직 포함되지 않습니다.',
    ...
    columns: { tenant_name: '공간', active_principals: '활성 주체', assistant_turns: 'Assistant Turns', agent_turns: 'Agent Turns', input_tokens: 'Input Token', output_tokens: 'Output Token', total_tokens: 'Total Token', mcp_calls: 'MCP Calls', mcp_success_rate: 'MCP Success Rate', mcp_avg_latency_ms: 'MCP Avg Latency', last_active: '최근 활동', model_id: '모델', model_type: '모델 유형', cache_read_tokens: 'Cache Read', cache_write_tokens: 'Cache Write', tool_name: '도구', calls: 'Calls', successful_calls: 'Successful Calls', success_rate: 'Success Rate', average_latency_ms: 'Average Latency', unattributed_calls: 'Unattributed Calls', knowledge_base_name: '지식 베이스', owner_tenant_name: '소유 공간', caller_tenant_name: '호출 공간', input: 'Input', output: 'Output', cacheRead: 'Cache Read', cacheWrite: 'Cache Write', successRate: 'Success Rate', avgLatency: 'Average Latency' },
  },
  platformApiKeys: {
    title: '플랫폼 API 키',
  ...
  integrations: {
    ...
    claw: {
      ...
      subtitle: `${branding.productName} REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.`,
      ...
      installCtaHint: `${branding.productName} Skill 설치 · 새 탭에서 열림`,
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: `자체 호스팅 ${branding.productName}와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.`,
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: `${branding.productName} 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.`
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      signedFlowDetail: `HMAC secret은 ${branding.productName}와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=weknora, exp(최대 24시간)가 필요합니다.`,
      ...
      hmacSecretDesc: `${branding.productName}에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.`,
  ...
  error: {
    ...
    requestTimeout: '요청 시간이 초과되었습니다. 다시 시도해 주세요',
  ...
  knowledgeList: {
    workspaceTitle: '{spaceName} · 지식베이스',
    create: '지식베이스 생성',
    filters: {
  ...
  auth: {
    ...
    firstTime: `${branding.productName}가 처음이신가요?`,
    ...
    registerSubtitle: `계정을 만들고 ${branding.productName}를 시작하세요`,
  ...
  settings: {
    ...
    sandbox: {
      ...
      templateLoadHint: `클러스터 연결을 입력한 뒤 새로고침하세요. ${branding.productName} 표준 템플릿이 없으면 「만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요.`,
      ...
      templateStepHint: `이 단계에서는 클러스터에 이미 있는 템플릿만 나열하며 자동으로 만들지 않습니다. 표준 템플릿이 없으면 「${branding.productName} 표준 템플릿 만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요. 템플릿이 준비된 뒤에 계속할 수 있습니다.`,
      loadingTemplates: 'Loading templates from the cluster...',
      templateBuildingHint: '표준 템플릿을 구축하는 중입니다. 목록이 자동으로 새로고침됩니다.',
      templateUntaggedHint: `빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 ${branding.productName}가 다시 빌드합니다.`,
      ...
      cubeDnsServersHelp: `선택 사항입니다. ${branding.productName} 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.`,
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: `모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 ${branding.productName}가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.`
      },
      welcome: {
        title: `${branding.productName}에 오신 것을 환영합니다`,
  ...
  navigation: { main: 'Main navigation', openMenu: 'Open navigation', portalHome: 'Home', newChat: 'New chat', recentChats: 'Recent chats', recentAll: 'All', recentCollapse: 'Collapse', knowledgeBases: '공간 지식베이스', agents: 'Agents', allAgents: 'All agents', mcpAccess: 'MCP 연결', members: '공간 멤버', managementCenter: '관리 센터', integrations: 'Integrations', workspaceSettings: 'Workspace Settings', usageAnalytics: 'Usage Analytics', modelsDefaults: 'Models & Defaults', runtime: 'Runtime', audit: 'Audit', systemSettings: 'System Settings', platformApiKeys: 'Platform API Keys', groups: { global: '글로벌', workspace: '지식 공간', management: '관리', knowledge: 'Knowledge', ai: 'AI', system: 'System Administration' } },
  spaceSwitcher: { currentSpace: 'Current space', search: 'Search spaces', current: 'Current space', unknown: 'Unknown space' },
  portalCard: { ask: 'Q&A', search: '지식 검색', responsible: '담당: {name}', unconfigured: '설정되지 않음', noSpaceDescription: '공간 설명이 없습니다' },
