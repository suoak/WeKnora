...
test('Portal defines a system-font typography hierarchy without content scaling', () => {
  ...
  assert.match(home, /padding:0 clamp\(24px,2vw,38px\) 48px/)
...
test('hero is a compact branded portal surface with legible KPI and search sizing', () => {
  ...
  assert.match(hero, /min-height:180px/)
  ...
  assert.match(hero, /hero-stats strong\{[^}]*font-size:32px/)
...
test('lifecycle rail keeps seven readable steps and removes nested stage cards', () => {
  ...
  assert.match(stage, /width:12px;height:12px/)
  assert.match(stage, /content:'›'/)
...
test('empty descriptions use presentation-only stage fallback and never render generic filler', () => {
  ...
  assert.match(map, /:fallback-description="stageFallbackDescription\(space\)"/)
  assert.match(map, /const stageFallbackDescriptions=stageShortDescriptions/)
