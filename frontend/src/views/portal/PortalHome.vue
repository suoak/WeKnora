    ...
    <main>
      ...
      <section class="discovery">
        ...
        <section v-else class="public-overview">
          ...
          <div><span>{{ t('portal.publicZone.eyebrow') }}</span><h2>{{ t('portal.publicZone.title') }}</h2><KnowledgeScaleSummary :stats="knowledgeSummary.publicArea" tone="compact" /><p>{{ t('portal.publicZone.description') }}</p></div>
        </section>
        ...
        <PortalSpaceGrid v-if="viewMode==='ipd'" :spaces="visibleSpaces" :loading="portal.loading" :empty-text="emptyText" :stage-labels="stageLabels"
          @request="openRequest" @enter="enterPortalSpace" @interaction="enterInteraction" />
        <div v-else-if="portal.loading" class="public-loading"><t-loading :text="t('portal.loadingSpaces')" /></div>
        <div v-else-if="!visibleSpaces.length" class="public-empty">{{ emptyText }}</div>
        <div v-else class="public-space-grid">
          <PublicKnowledgeSpaceCard v-for="space in visibleSpaces" :key="space.tenant_id" :space="space"
            @request="openRequest" @enter="enterPortalSpace" @interaction="enterInteraction" />
        </div>
        <div v-if="portal.spaces.length>PAGE_SIZE" class="result-pagination">
          ...
          <t-button v-if="hasMoreSpaces" variant="outline" @click="showMoreSpaces">{{ t('portal.loadMoreSpaces') }}</t-button>
...
import PublicKnowledgeSpaceCard from '@/components/portal/PublicKnowledgeSpaceCard.vue'
import KnowledgeScaleSummary from '@/components/portal/KnowledgeScaleSummary.vue'
...
const { t }=useI18n()
const PUBLIC_CATEGORY=PUBLIC_KNOWLEDGE_CATEGORY
const IPD_PAGE_SIZE=12
...
const visibleSpaces=computed(()=>viewMode.value==='public'
  ? visiblePublicKnowledgeSpaces(portal.spaces,visibleLimit.value>PUBLIC_KNOWLEDGE_PREVIEW_LIMIT)
  : portal.spaces.slice(0,visibleLimit.value))
...
function showMoreSpaces(){
  visibleLimit.value=viewMode.value==='public'?portal.spaces.length:visibleLimit.value+IPD_PAGE_SIZE
}

watch(()=>portal.search,()=>{visibleLimit.value=PAGE_SIZE.value;window.clearTimeout(searchTimer);searchTimer=window.setTimeout(()=>void portal.loadSpaces(),320)})
watch([()=>portal.selectedStage,()=>portal.selectedCategory],()=>{visibleLimit.value=PAGE_SIZE.value;void portal.loadSpaces()})

function selectView(mode:'ipd'|'public'){
  viewMode.value=mode
  visibleLimit.value=mode==='public'?PUBLIC_KNOWLEDGE_PREVIEW_LIMIT:IPD_PAGE_SIZE
