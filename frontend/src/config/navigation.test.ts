...
test('viewer and contributor see global and accessible workspace navigation only', () => {
  for (const role of ['viewer', 'contributor']) {
    ...
    assert.deepEqual(entries.map((entry) => entry.id), ['portal', 'knowledge-bases', 'agents', 'mcp-access', 'members'])
...
test('management center is visible to workspace admins and system admins', () => {
  ...
  assert.deepEqual(admin.map((entry) => entry.id), ['portal', 'knowledge-bases', 'agents', 'mcp-access', 'members', 'management-center'])

  const systemAdmin = visibleNavigationEntries({ role: 'viewer', isSystemAdmin: true, supports: supported })
  assert.deepEqual(systemAdmin.map((entry) => entry.id), ['portal', 'knowledge-bases', 'agents', 'mcp-access', 'members', 'management-center'])
})
...
test('capabilities and active settings section are respected', () => {
  ...
  assert.deepEqual(NAVIGATION_REGISTRY.map((entry) => entry.id), ['portal', 'knowledge-bases', 'agents', 'mcp-access', 'members', 'management-center'])
})
