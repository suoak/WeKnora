      ...
      <section class="nav-group">
        ...
        <t-tooltip v-else :content="t(entry.labelKey)" placement="right" :disabled="!collapsed">
          <button type="button" :data-guide="`nav-${entry.id}`"
            ...
            <span v-if="entry.id === 'knowledge-bases'" class="nav-entry__knowledge-icon" aria-hidden="true" />
            <t-icon v-else :name="entry.icon" /><span v-if="!collapsed">{{ t(entry.labelKey) }}</span>
          </button>
        </t-tooltip>
        </template>
      </section>
