<template>
  ...
  <article v-else-if="space" class="space-summary" :class="[variant, space.access_state, { current: isActiveSpace }]"
    ...
    <div class="summary-main">
      ...
      <div class="space-title"><span class="space-icon"><t-icon :name="visualIcon" /><i v-if="space.access_state === 'discoverable'"><t-icon name="lock-on" /></i></span><strong :title="space.display_name">{{ space.display_name }}</strong><span v-show="isActiveSpace">{{ t('spaceSwitcher.current') }}</span></div>
    ...
    <div v-if="space.access_state === 'accessible'" class="accessible-actions" @click.stop @keydown.stop>
      <div class="icon-actions">
        <t-tooltip :content="actionLabel('ask')"><button type="button" :title="actionLabel('ask')" :aria-label="actionLabel('ask')" @click="$emit('ask', space)"><t-icon name="chat" /><span v-if="variant === 'normal'">{{ t('portalMap.ask') }}</span></button></t-tooltip>
        <t-tooltip :content="actionLabel('search')"><button type="button" :title="actionLabel('search')" :aria-label="actionLabel('search')" @click="$emit('search', space)"><t-icon name="search" /><span v-if="variant === 'normal'">{{ t('portalExperience.quickActions.search') }}</span></button></t-tooltip>
      </div>
...
const props = withDefaults(defineProps<{
  ...
  visualIcon?: string
}>(), { variant: 'normal', loading: false, isActiveSpace: false, stageNumber: '', stageName: '', fallbackDescription: '', visualIcon: 'folder' })
