<template>
  ...
  <article v-else-if="space" class="space-summary" :class="[variant, space.access_state, { current: isActiveSpace }]"
    ...
    <div class="summary-main">
      ...
      <t-tooltip v-if="!suppressResponsible && variant === 'compact'" :content="responsibleLabel"><span class="space-responsible is-compact"><t-icon name="user" />{{ displayResponsible }}</span></t-tooltip>
      <span v-else-if="!suppressResponsible" class="space-responsible"><t-icon name="user" />{{ responsibleLabel }}</span>
