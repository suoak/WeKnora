<template>
  ...
  <article v-else-if="space" class="space-summary" :class="[variant, space.access_state, { current: isActiveSpace }]"
    ...
    <div class="summary-main">
      ...
      <div class="space-title" :class="{ 'is-title-hidden': hideTitle }">
        <div class="space-title__identity">
          <span v-if="variant !== 'flat'" class="space-icon"><t-icon :name="visualIcon" /><i v-if="space.access_state === 'discoverable'"><t-icon name="lock-on" /></i></span>
    ...
    <div v-if="space.access_state === 'accessible'" class="accessible-actions" @click.stop @keydown.stop>
      <div class="icon-actions">
        <t-tooltip :content="actionLabel('ask')"><button type="button" :title="actionLabel('ask')" :aria-label="actionLabel('ask')" @click="$emit('ask', space)"><t-icon name="chat" /><span>{{ actionLabel('ask') }}</span></button></t-tooltip>
        <t-tooltip :content="actionLabel('search')"><button type="button" :title="actionLabel('search')" :aria-label="actionLabel('search')" @click="$emit('search', space)"><t-icon name="search" /><span>{{ actionLabel('search') }}</span></button></t-tooltip>
      </div>
...
const props = withDefaults(defineProps<{
  ...
  suppressResponsible?: boolean
}>(), { variant: 'normal', loading: false, isActiveSpace: false, stageNumber: '', stageName: '', fallbackDescription: '', visualIcon: 'folder', hideTitle: false, suppressDescription: false, suppressResponsible: false })
