<template>
  <div ref="root" class="space-switcher" data-guide="space-switcher">
    <button type="button" class="space-switcher__trigger" :class="{ 'is-readonly': !canSwitch }"
      ...
      <t-icon v-if="canSwitch" name="chevron-down" />
    </button>
    <div v-if="open && canSwitch" class="space-switcher__panel">
...
function toggle() { if (canSwitch.value) open.value = !open.value }
...
.space-switcher__trigger.is-readonly{cursor:default}.space-switcher__trigger.is-readonly:hover{border-color:color-mix(in srgb,var(--td-brand-color) 12%,var(--td-component-stroke))}
</style>
