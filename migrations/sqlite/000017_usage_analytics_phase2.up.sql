CREATE INDEX idx_model_usage_channel_time ON model_usage_events (channel, occurred_at DESC);

DROP INDEX idx_mcp_usage_tenant_time;
DROP INDEX idx_mcp_usage_tool_time;
ALTER TABLE mcp_usage_events RENAME TO mcp_usage_events_phase1;
...
SELECT id, event_key, caller_tenant_id, principal_type, principal_id, api_key_id,
    direction, tool_name, client_name, client_version, transport, success,
    error_code, latency_ms, request_id, trace_id, occurred_at, created_at
FROM mcp_usage_events_phase1;

DROP TABLE mcp_usage_events_phase1;
...
CREATE INDEX idx_mcp_usage_direction_time ON mcp_usage_events (direction, occurred_at DESC);
CREATE INDEX idx_mcp_usage_service_time ON mcp_usage_events (mcp_service_id, occurred_at DESC);
